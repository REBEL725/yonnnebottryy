...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./YoneRobot/resources/font/*")
    ...
    fname = "LogoByYone.png"
    img.save(fname, "png")
    await pesan.edit('`Done`')
    await telethn.send_file(event.chat_id, file=fname, caption = f"ᗰᗩᗪᗴ ᗷY [ᗰIՏՏ ᑫᑌᗴᗴᑎ](t.me/MISS_QUEENS_BOT)")
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error Report @REBEL_BOT_CHATING, {e}')
